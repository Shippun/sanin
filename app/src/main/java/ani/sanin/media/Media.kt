...
import ani.sanin.connections.anilist.api.MediaExternalLink
import ani.sanin.connections.anilist.api.ExternalLinkType
import ani.sanin.connections.anilist.api.MediaList
import ani.sanin.connections.anilist.api.MediaStreamingEpisode
import ani.sanin.connections.anilist.api.MediaType
import ani.sanin.connections.anilist.api.Query
import ani.sanin.connections.mal.MAL
import ani.sanin.connections.mal.MalAnimeNode
import ani.sanin.connections.mal.MalListEntry
import ani.sanin.connections.mal.JikanMediaData
import ani.sanin.connections.mal.JikanBroadcast
import ani.sanin.connections.mal.JikanRelationEntry
...
import ani.sanin.connections.anilist.api.Media as ApiMedia
...
data class Media(
    ...
    constructor(node: MalAnimeNode, isAnime: Boolean) : this(
        ...
        countryOfOrigin = "JP",
        userStatus = if (node.myListStatus?.isRewatching == true)
            "REPEATING"
        else
            convertMalStatusToAnilist(node.myListStatus?.status, true),
        userProgress = node.myListStatus?.numEpisodesWatched,
        ...
        this.shareLink = "https://myanimelist.net/anime/${node.id}"
    ...
    constructor(entry: MalListEntry, isAnime: Boolean) : this(entry.node, isAnime) {
        ...
        this.userProgress = ls?.numEpisodesWatched
        this.userScore = ls?.score?.times(10) ?: 0
        this.userStatus = if (ls?.isRewatching == true)
            "REPEATING"
        else
            convertMalStatusToAnilist(ls?.status, true)
    ...
    constructor(jikan: JikanMediaData, isAnime: Boolean) : this(
        ...
        fun mapRelationEntry(entry: JikanRelationEntry?, relation: String?): Media? {
            ...
            return Media(
                ...
                anime = if (isRelAnime) Anime(null, null, null) else Anime(),
            ).apply {
                val relText = relation?.uppercase(Locale.US)?.replace(" ", "_")
        ...
        withContext(Dispatchers.IO) {
            this@deleteFromList?.let { media ->
                if (rescueMode) {
                    ...
                    val pending = ani.sanin.connections.PendingDeletion(
                        mediaId = media.id,
                        idMAL = media.idMAL,
                        isAnime = media.anime != null,
                    )
                    val existing: List<ani.sanin.connections.PendingDeletion> =
